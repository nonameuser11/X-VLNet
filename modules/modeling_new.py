...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def _loose_similarity(self, sequence_output,seq_features, visual_output, attention_mask, video_mask, sim_header="meanP"):
        ...
        indices_visual_thrsh = indices_visual[sorted_global_visual<self.score_threshold]
        indices_sequence_thrsh = indices_sequence[sorted_global_sequence<self.score_threshold]
        ...
        #global_video_sentence_loss = (1 - beta)*loss_i2t + beta * loss_t2i
        '''
        ...
        word_features_pooled = word_features_pooled.permute(1,2,0) # b_t x b_v x dim
        ...
        #logits = (retrieve_logits+finegrained_logits)/2
        
        #logits = video_sentence_logits 
        #return video_sentence_logits,finegrained_logits
        retrieve_logits = (global_logits+frame_word_logits) / 2 
        
        if self.training:
            finegrained_logits = alpha * s2f_logits + (1-alpha)*v2w_logits 
            return global_video_sentence_loss,retrieve_logits,finegrained_logits
        #finegrained_logits = torch.matmul(alpha*s2f_logits,((1-alpha)*v2w_logits))
        
        return global_video_sentence_loss,retrieve_logits,0
    ...
    def get_similarity_logits(self, sequence_output,seq_features, visual_output, attention_mask, video_mask, shaped=False, loose_type=False):
        ...
        if loose_type:
            ...
            global_video_sentence_loss,retrieve_logits,fine_grained_logits = self._loose_similarity(sequence_output,seq_features, visual_output, attention_mask, video_mask, sim_header=self.sim_header)
        else:
            assert self.sim_header in ["tightTransf"]
            retrieve_logits = self._cross_similarity(sequence_output, visual_output, attention_mask, video_mask, )

        return retrieve_logits,global_video_sentence_loss,fine_grained_logits, contrastive_direction
    #sequence_output,seq_features, visual_output, input_mask, video_mask,loose_type=model.loose_type
    
    def get_balow_twins_logits(self,sentence_output,video_output):
        B,D = sentence_output.shape 
        c = torch.einsum('ac,ad->cd',sentence_output,video_output) / B # Dimension x Dimension 
        #loss
        on_diag = torch.diagonal(c).add_(-1).pow_(2).sum()
        off_diag = c.flatten()[1:].view(D-1,D+1)[:,:-1].pow_(2).sum()
        cdcr_loss = (on_diag + off_diag * 0.06)
        return cdcr_loss
        
    # def get_final_logits(self, sequence_output,seq_features, visual_output, attention_mask, video_mask,shaped=False, loose_type=False):
    #     if shaped is False:
    #         attention_mask = attention_mask.view(-1,attention_mask.shape[-1])
    #         video_mask = video_mask.view(-1,video_mask.shape[-1])
    #     if loose_type:
